...
let INPUTS: {[name: string]: InputFeature} = {
  "x": {f: (x, y) => x, label: "X1"},
  "y": {f: (x, y) => y, label: "X2"},
  "xSquared": {f: (x, y) => x * x, label: "X1^2"},
  "ySquared": {f: (x, y) => y * y,  label: "X2^2"},
  "xTimesY": {f: (x, y) => x * y, label: "X1*X2"},
  "sinX": {f: (x, y) => Math.sin(x), label: "sin(X1)"},
  "sinY": {f: (x, y) => Math.sin(y), label: "sin(X2)"},
};
